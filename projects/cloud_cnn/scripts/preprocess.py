...
def run(
    ...
    for data_filename, label_filename, n_tiles in data_df.values:
        ...
        input_bands = conf.input_bands
        if image.shape[0] == 4:
            input_bands = ['Blue', 'Green', 'Red', 'NIR1']

        # Calculate indices and append to the original raster
        image = indices.add_indices(
            xraster=image, input_bands=input_bands,
            output_bands=conf.output_bands)
        logging.info(f'Indices - Image: {image.shape}, Label: {label.shape}')

        # Lower the number of bands if required
        image = modify_bands(
            xraster=image, input_bands=input_bands,
            output_bands=conf.output_bands)
        logging.info(f'Modify - Image: {image.shape}, Label: {label.shape}')
